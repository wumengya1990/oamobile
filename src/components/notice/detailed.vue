  ...
  <div class="detailed" ref="layerPageHeight">
    ...
    <div class="detailedContent">
        ...
        <ul>
          <li>
            ...
            <div class="rightCon">
              <p>
                <span
                  :style="{color:outFileDetails.notice_Type=='会议通知'?'#F30':'#333'}"
                >{{$route.params.listType==0 ? fileDetails.notice_Type : outFileDetails.notice_Type }}</span>
              </p>
            </div>
          </li>
          <li>
            <em>发送人：</em>
            <div class="rightCon">
              <p>{{$route.params.listType==0 ? fileDetails.userName : outFileDetails.userName }}</p>
            </div>
          </li>
          <li>
            <em>发送日期：</em>
            <div class="rightCon">
              <p>{{$route.params.listType==0 ? fileDetails.beginDate : outFileDetails.beginDate|newBeginDate}}</p>
            </div>
          </li>
          <li>
            <em>通知详情：</em>
            <div class="rightConNoOver">
              ...
              <iframe
                :src="$route.params.listType==0 ? fileDetails.content : outFileDetails.content"
                frameborder="0"
                width="100%"
                height="100%"
                scrolling="auto"
          ...
          <li>
            <div class="detailsState">
              <van-button type="info" plain hairline size="small" v-if="$route.params.listType==1">
                <van-icon name="eye" />
                阅读情况{{readNumbeState.total - readNumbeState.unread}}/{{readNumbeState.total}}
              </van-button>
              <van-button
                type="info"
                plain
                hairline
                size="small"
                v-if="$route.params.listType==1&&$route.params.mesType=='会议通知'"
                @click="submitDetails"
              >
                <van-icon name="column" />查看报送情况
              </van-button>
              <van-button
                type="info"
                plain
                hairline
                size="small"
                v-if="($route.params.listType==0&&$route.params.mesType==1)||($route.params.dandu==1&&fileDetails.notice_Type=='会议通知')"
                @click="openSBLayer"
              >
                <van-icon name="column" />报送
              </van-button>
            </div>
          </li>
        </ul>
      ...
      <div class="detailsBox attachment" v-if="fujianName.length!=0">
        ...
        <div class="attachmentList">
          <ul>
            <li
              v-for="(fj, index) in fujianName"
              :key="index"
              @click="panduanImg(fj,fujianURL[index])"
            >
              <div class="fileImg">
                ...
                <dl>
                  ...
                  <dt>
                    <a href="javascript:void(0)">
                      <i class="icon iconfont iconxiazai"></i>
                    </a>
                  </dt>
                  <dd>200K</dd>
                </dl>
      ...
      <div class="detailsBox replyBox">
        ...
        <div v-if="$route.params.listType==0">
          ...
          <div class="bts">
            <van-button
              type="primary"
              style="height:40px; font-size:1.1rem; width:95%; display:block; margin:0 auto;"
              @click="huifu"
            >回复</van-button>
          </div>
        </div>
    ...
    <van-popup v-model="shangbaoShow" position="right">
      <div class="remarksBox">
        <p>
          <span style="color:#F30; padding:0 5px 0 0;">备注:</span>报送截止时间为
          <span
            style="color:#F30; padding:0 5px"
          >{{fileDetails.sign_Up_EndTime|newsign_Up_EndTime}}</span>,在此时间点前可进行报送修改
        </p>
      </div>
          ...
          <table class="tableStyle" width="100%" cellpadding="0" cellspacing="0">
            <thead>
              ...
              <tr v-for="(p,index) in canhuiPeoList" :key="index">
                ...
                <td>{{p.post}}</td>
                <td width="30%">
                  <van-button size="mini" @click="dropPep(p.autoID,index)" type="danger">删除</van-button>
                </td>
              </tr>
            </thead>
          </table>
...
export default {
  ...
  methods: {
    ...
    loadxaingqing() {
      ...
      if (me.$route.params.listType == 0) {
        ...
        me.$api.get(url, params, res => {
          ...
          me.readNumbeState.unread = res.unread;
          //me.replyList = res.replyList;
          let file = me.outFileDetails.viewPath;
          let furl = res.data.pathBase;
          let aa=furl.split('/')
          me.dpfurl=aa[0]+'//'+aa[2]
          if (file == "" || file == null || file == undefined) {
    ...
    huifu() {
      ...
      let params = {
        autoID: me.$route.params.tzid,
        path: me.filePath,
        feedBackIdea: me.backMessage
      };
    ...
    canhuiAdd(action, done) {
      ...
      if (action === "confirm") {
        if (!me.luruRenyuan.username || !me.luruRenyuan.userjob) {
          ...
          let params = {
            pAutoID: me.$route.params.tzid,
            userName: me.luruRenyuan.username,
            post: me.luruRenyuan.userjob,
            s_Type: 0
          };
          me.$api.post(url, params, res => {
            // console.log(res);
